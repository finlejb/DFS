...
#-------------------------------------------------------------------------#
#ADD AVERAGE TAv TO EACH LINEUP
#-------------------------------------------------------------------------#

for n in range(len(all_teams)):
    try:
        print('Adding average TAv to',str(all_teams2[n]),'lineup...')
        ...
        if lineup == []:
            print('Lineup was [] for',str(all_teams2[n]))
            continue

        handedness = lineup[0][3]
        ...
        for row in lineup:
            player = row[0]
            for row in bat_stats:
                if row[0] == player:
                    TAvs.append(row[26])

        for i in range(len(TAvs)):
            TAvs.append(float(TAvs[i]))
        for i in range(int(len(TAvs)/2)):
            del(TAvs[0])

        TAv_avg = numpy.mean(TAvs)

        lineup.append(['Avg TAv' , TAv_avg])
